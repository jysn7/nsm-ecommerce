...
export default function LoginPage() {
  ...
  const { email, setEmail, password, setPassword, errors, validate } = useAuthStore()
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh]">
      <Card className="w-full max-w-sm rounded-xl border-border bg-background shadow-lg shadow-zinc-200/10 overflow-hidden">
        <CardHeader className="px-6 pt-6 pb-2 text-center">
          <CardTitle className="text-xl font-normal tracking-tight text-foreground">Login</CardTitle>
        </CardHeader>
        
        <CardContent className="grid gap-4 px-6 pb-4 text-center">
          <Button 
            variant="outline" 
            onClick={handleGoogleLogin}
            disabled={loading}
            className="h-10 w-full cursor-pointer rounded-xl border-border font-normal hover:bg-secondary transition-colors"
          >
          ...
          <form onSubmit={handleSubmit} className="grid gap-3 text-left">
            <div className="grid gap-1">
              ...
              <Input 
                ...
                disabled={loading}
                className={`h-10 rounded-xl border-none bg-muted/50 px-4 focus-visible:ring-1 ${errors.email ? 'ring-1 ring-destructive' : 'focus-visible:ring-ring'}`}
              />
            </div>
            ...
            <div className="grid gap-1">
              ...
              <Input 
                ...
                placeholder="••••••••"
                disabled={loading}
                className={`h-10 rounded-xl border-none bg-muted/50 px-4 focus-visible:ring-1 ${errors.password ? 'ring-1 ring-destructive' : 'focus-visible:ring-ring'}`}
              />
            </div>
